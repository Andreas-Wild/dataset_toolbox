"""Lightbox component for NiceGUI — a thumbnail grid with full-screen preview.
...
class Lightbox:
    ...
    def _handle_key(self, event_args: events.KeyEventArguments) -> None:
        ...
        if event_args.key.escape:
            self.dialog.close()
        if self.large_image.source not in self.image_list:
            return
    ...
    def clear(self) -> None:
        """Remove all images from the lightbox."""
