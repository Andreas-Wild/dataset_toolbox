            ...
            with ui.row().classes("gap-6"):
                ...
                with (
                    ...
                    with ui.column().classes("items-center p-4 gap-2 h-full"):
                        ...
                        ui.label(
                            "Interactively edit masks for image datasets."
                        ).classes("text-lg text-gray-400 text-center")

                with (
                    ui.card()
                    .on("click", lambda: ui.navigate.to("/viewer"))
                    .classes("cursor-pointer hover:animate-pulse size-55")
                ):
                    with ui.column().classes("items-center p-4 gap-2 h-full"):
                        ui.icon("visibility", size="xl")
                        ui.label("OME TIFF Viewer").classes("text-lg font-semibold")
                        ui.label("View OME-TIFF images with mask overlays.").classes(
                ...
                with (
                    ui.card()
                    .on("click", lambda: ui.navigate.to("/converter"))
                    .classes("cursor-pointer hover:animate-pulse size-55")
                ):
                    with ui.column().classes("items-center p-4 gap-2 h-full"):
                        ui.icon("swap_horizontal_circle", size="xl")
                        ui.label("OME Converter").classes("text-lg font-semibold")
